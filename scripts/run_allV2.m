...
for p = 1:length(file_osim_array)
    ...
    data.Mass = massmat(p); % our c3d files didn't have mass data, so we manually insert them
    ...
    if run_opensim_algorithms == 0
        ...
        %         reqFiles.RRAconstraintFile     = 'controlconstraints.xml';
    ...
    system(command);
    DCfsolvePower = ['C:\Users\lsboe\Documents\NiksWerkt\direct_collocation_files\DCResults\' data.Name '_fsolve_Actuation_Power.sto'];
    ...
    command = ['analyze -S ' data.Name '_Setup_Analysis.xml'];
    system(command);
    DCfsolvePower = ['C:\Users\lsboe\Documents\NiksWerkt\fwd\DCResults\' data.Name '_Fminco_Actuation_Power.sto'];
    powerFminDat = readMOT(DCfsolvePower);
    MaxAbsTargetGRF = repmat(max(abs(desiredFPs),[],1),nNodes,1);
...
for i = 1:6
    ...
    if i == 4 || i == 5 || i == 6
        ...
        fileTBD.ForceSet.objects.TorqueActuator(i-3).optimal_force = '2';
