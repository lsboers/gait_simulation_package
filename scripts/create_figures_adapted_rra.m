...
%%%%%%%%%%%%%%%%%%%
data_TSI      = data_TSI([1,3,6,7,9,11,13,15,17,18]);
ground_RO = ground_RO([1,3,6,7,9,11,13,15,17,18]);
data_RO    = data_RO([1,3,6,7,9,11,13,15,17,18]);
ground_TSI   = ground_TSI([1,3,6,7,9,11,13,15,17,18]);
%%%%%%%%%%%%%%%%%%%
...
if exist('datamat.mat','file')
    for i = 1:num_subjects
        ...
        name_current_subject = data_RO(i).name(idx_name_subject:end-8);
        file_osim = [name_current_subject '_RAD_MARKED.osim'];
        current_osim_model = Model(file_osim);
...
%         
        ...
        idxRBExp2 = [totIdxTBD2(end):idxElLast];
        idxRBExp1 = [totIdxTBD1(end):idxRbLast];
        ...
        idxElLast = find(totSumTBD(idx_diff_rb2:end) <= 0, 1, 'first') + idx_diff_rb2;
        idxRbLast = find(totSumTBD(idx_diff_rb1:end) <= 0, 1, 'first') + idx_diff_rb1;
        idxRBFsl2 = [totIdxTBD2(end):idxElLast];
        idxRBFsl1 = [totIdxTBD1(end):idxRbLast];
        ...
        idxElLast = find(totSumTBD(idx_diff_rb2:end) <= 0, 1, 'first') + idx_diff_rb2;
        idxRbLast = find(totSumTBD(idx_diff_rb1:end) <= 0, 1, 'first') + idx_diff_rb1;
        idxRBFmc2 = [totIdxTBD2(end):idxElLast];
        idxRBFmc1 = [totIdxTBD1(end):idxRbLast];
...
% %         datatracking.JworkRb(i).fp = workFpRbJoint;
    ...
    save('datamat2', 'labels_power', 'labelsKin', 'datatracking')
...
try
    ...
    for i = 1:size(plotKins,2)+1
        ...
        if i == size(plotKins,2)+1
            hL = legend({'±1 Std. RRA','Average RO'});
...
%       set(hPieComponentHandles(k*2), 'String', num2str(wrk(k)), 'FontSize', 24 );
    ...
    if i == 1
        ...
        P(10).String = [P(10).String, '\bf**'];
        P(6).String = [P(6).String, '\bf**'];
    end
%     P = pie(wrk/sum(wrk)); 
    wrks(i,:) =wrk/sum(wrk);
    title(plotLabels(i));
